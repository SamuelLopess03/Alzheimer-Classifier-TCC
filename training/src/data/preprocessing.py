...
class MedicalImagePreprocessor:
    ...
    def _get_architecture_config(self) -> Dict:
        if self.architecture_name not in _ARCHITECTURE_CONFIGS:
            print(f"\nArquitetura '{self.architecture_name}' não reconhecida. Usando config padrão (ResNeXt50).\n")
            return _ARCHITECTURE_CONFIGS["resnext50_32x4d"]
        return _ARCHITECTURE_CONFIGS[self.architecture_name]
