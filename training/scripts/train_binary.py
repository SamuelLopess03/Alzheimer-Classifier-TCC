...
def execute_train_binary() -> bool:
    try:
        ...
        if not os.path.exists(train_path):
            ...
            print("   python scripts/train_binary.py --prepare-data\n")
            return False
...
def run_pipeline(args):
    print_banner(
        "PIPELINE DE TREINAMENTO BINÁRIO",
        "Alzheimer Detection System - Binary Model"
    )
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
                    Exemplos de uso:
                      ...
                      python train_binary.py --prepare-data
                    
                      # Apenas treinamento
                      python train_binary.py --train
                    
                      # Apenas inferência com Grad-CAM
                      python train_binary.py --inference --generate-gradcam --gradcam-samples 15
                    
                      # Pipeline completa com Grad-CAM
                      python train_binary.py --generate-gradcam --gradcam-samples 20
               """
    )
    ...
    pipeline_group.add_argument(
        '--train',
        action='store_true',
        help="Executar treinamento do modelo binário"
    )
    pipeline_group.add_argument(
        '--inference',
        action='store_true',
